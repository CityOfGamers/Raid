	...
	void OnGUI()
	{
		...
		if (winMode == WindowMode.Part1) {
			...
			if (ambientState) {
				...
				if(ambientLight == AmbientLight.HDR)
				{
				...
				if (ambientLightRef != ambientLight || skyCubeRef != skyCube
					...
					|| gradientDiffusionRef != gradientDiffusion )
				{
					helper.Update_Ambient (Ambient_Enabled,ambientLight,skyCube, skyExposure,hdrRotation,skyTint,groundColor,tickness
			...
			if (sunState) {
				...
				sunIntensity = EditorGUILayout.Slider ("Intenity", sunIntensity, 0, 10f);
			...
			if (lightSettingsState) {
				...
				EditorGUILayout.Space ();
				indirectDiffuse = EditorGUILayout.Slider ("Indirect Diffuse", indirectDiffuse,0,1f);
				indirectSpecular = EditorGUILayout.Slider ("Indirect Specular", indirectSpecular,0,1f);
				EditorGUILayout.Space ();

				if (lightingModeRef != lightingMode || Scene_EnabledRef != Scene_Enabled || indirectDiffuseRef != indirectDiffuse
					|| indirectSpecularRef != indirectSpecular) {
					//----------------------------------------------------------------------
					// Update Lighting Mode
					helper.Update_LightingMode (Scene_Enabled,lightingMode,indirectDiffuse,indirectSpecular);
					//----------------------------------------------------------------------
					if (LB_LightingProfile)
					{
			...
			{
				helper.Update_GlobalFog(Fog_Enabled,fogMode,fogColorMode,fogColor,fogDistance,fogHeight,fogStart,fogEnd,fogDensity
					,scatteringAlbedo,freePath,fogAnisotropy,LightProbeDimmer);
				if (LB_LightingProfile)
				{
			...
			//---------------------------------------------------------------------------------------
				...
				if (fogState) {
					if (helpBox)
						EditorGUILayout.HelpBox ("Set Fog Mode Linear or Exponential  type", MessageType.Info);

					var fogModeRef = fogMode;
					var fogColorModeRef = fogColorMode;
					var fogDistanceRef = fogDistance;
					var fogHeightRef = fogHeight;
					var fogStartRef = fogStart;
					var fogEndRef = fogEnd;
					var fogColorRef = fogColor;
				var fogDensityRef = fogDensity;

				var scatteringAlbedoRef = scatteringAlbedo;
				var freePathRef = freePath;
				var fogAnisotropyRef = fogAnisotropy;
				var LightProbeDimmerRef = LightProbeDimmer;

					// choose ambient lighting mode   (color or skybox)
					fogMode = (CustomFog)EditorGUILayout.EnumPopup ("Fog Type", fogMode, GUILayout.Width (343));

					if(fogMode == CustomFog.Exponential)
					{
						fogColorMode = (FogColorType)EditorGUILayout.EnumPopup ("Color Mode", fogColorMode, GUILayout.Width (343));
					EditorGUILayout.Space();
					fogColor = EditorGUILayout.ColorField (new GUIContent("Color"), fogColor,true,false,true,new ColorPickerHDRConfig(-10f,10f,-10f,10f),null);
					EditorGUILayout.Space();
					fogDensity	 = EditorGUILayout.Slider ("Density", fogDensity, 0, 1);
					EditorGUILayout.Space();
					fogDistance = EditorGUILayout.Slider ("Distance", fogDistance, 0, 360);
						fogHeight = EditorGUILayout.Slider ("Height", fogHeight, 0, 360);
					}
					...
					fogDensity	 = EditorGUILayout.Slider ("Density", fogDensity, 0, 1);
					EditorGUILayout.Space();
						fogStart = EditorGUILayout.Slider ("Start", fogStart, 0, 200);
						fogEnd = EditorGUILayout.Slider ("End", fogEnd, 0, 360);
						fogHeight = EditorGUILayout.Slider ("Height", fogHeight, 0, 360);
					}
				if(fogMode == CustomFog.Volumetric)
				{
					scatteringAlbedo = EditorGUILayout.ColorField (new GUIContent("Scattering Albedo"), scatteringAlbedo,true,false,true,new ColorPickerHDRConfig(-10f,10f,-10f,10f),null);
					EditorGUILayout.Space();
					freePath	 = EditorGUILayout.FloatField ("Free Path", freePath);
					EditorGUILayout.Space();
					fogAnisotropy = EditorGUILayout.Slider ("Anisotropy", fogAnisotropy, -1f, 1f);
					LightProbeDimmer = EditorGUILayout.Slider ("Light Probe Dimmer", LightProbeDimmer, 0, 1f);
				}

					if (fogModeRef != fogMode || fogColorModeRef != fogColorMode
						|| fogDistanceRef != fogDistance || fogHeightRef != fogHeight
						|| fogStartRef != fogStart || fogEndRef != fogEnd
					|| fogColorRef != fogColor || fogDensityRef != fogDensity
					|| scatteringAlbedoRef != scatteringAlbedo || freePathRef != freePath 
					|| fogAnisotropyRef != fogAnisotropy || LightProbeDimmerRef != LightProbeDimmer)
					{
					helper.Update_GlobalFog(Fog_Enabled,fogMode,fogColorMode,fogColor,fogDistance,fogHeight,fogStart,fogEnd,fogDensity
						,scatteringAlbedo,freePath,fogAnisotropy,LightProbeDimmer);
						if (LB_LightingProfile)
						{
							...
							LB_LightingProfile.fogColorMode = fogColorMode;
							LB_LightingProfile.fogDistance = fogDistance;
							LB_LightingProfile.fogHeight = fogHeight;
							LB_LightingProfile.fogStart = fogStart;
							LB_LightingProfile.fogEnd = fogEnd;
							LB_LightingProfile.fogColor = fogColor;
						LB_LightingProfile.fogDensity = fogDensity;

						LB_LightingProfile.scatteringAlbedo = scatteringAlbedo;
						LB_LightingProfile.freePath = freePath;
						LB_LightingProfile.fogAnisotropy = fogAnisotropy;
						LB_LightingProfile.LightProbeDimmer = LightProbeDimmer;
			...
			var dofDistance2Ref = dofDistance2;

			if(dofState)
			{
				dofDistance2 = (float)EditorGUILayout.Slider ("Distance", dofDistance2, 0, 0.07f);
			}

			if (DOF_EnabledRef != DOF_Enabled || dofDistance2Ref != dofDistance2) {

			helper.Update_DOF(DOF_Enabled,dofDistance2);

			//----------------------------------------------------------------------
			if (LB_LightingProfile)
			{
					LB_LightingProfile.DOF_Enabled = DOF_Enabled;
					LB_LightingProfile.dofDistance2 = dofDistance2;
					EditorUtility.SetDirty (LB_LightingProfile);
			}
		}
		...
		if (LB_LightingProfile) {
			...
			fogDistance = LB_LightingProfile.fogDistance;
			fogHeight = LB_LightingProfile.fogHeight;
			fogStart = LB_LightingProfile.fogStart;
			fogEnd = LB_LightingProfile.fogEnd;
			fogColor = LB_LightingProfile.fogColor;
			fogDensity = LB_LightingProfile.fogDensity;

			scatteringAlbedo = LB_LightingProfile.scatteringAlbedo;
			freePath = LB_LightingProfile.freePath;
			fogAnisotropy = LB_LightingProfile.fogAnisotropy;
			LightProbeDimmer = LB_LightingProfile.LightProbeDimmer;

			// DOF 2
				dofDistance2 = LB_LightingProfile.dofDistance2;
