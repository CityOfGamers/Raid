    ...
    public class GaiaManagerEditor : EditorWindow, IPWEditor
    {
        ...
        void StandardTab()
        {
                ...
                m_settings.m_currentEnvironment == GaiaConstants.EnvironmentTarget.PowerfulDesktop)
            {
                ...
                if (UnityEditor.PlayerSettings.colorSpace != ColorSpace.Linear || tier1.renderingPath != RenderingPath.DeferredShading)
                {
                    if (m_editorUtils.ButtonAutoIndent("0. Set Linear Deferred"))
                    {
                        ...
                        m_editorUtils.GetTextValue("Yes"), m_editorUtils.GetTextValue("Cancel")))
                        {
                            ...
                            UnityEngine.RenderSettings.defaultReflectionResolution = 256;
                            ...
                            {
                                UnityEngine.RenderSettings.sun = GameObject.Find("Directional light").GetComponent<Light>();
                            }
                        }
                    }
                }
            ...
            if (m_editorUtils.ButtonAutoIndent("4. Bake Lighting"))
            {
                if(!DisplayErrorIfInvalidTerrainCount(1))
                {
                    ...
                    m_editorUtils.GetTextValue("Bake"), m_editorUtils.GetTextValue("Cancel")))
                    {
                        ...
                        if (UnityEngine.RenderSettings.ambientMode != AmbientMode.Skybox)
                        {
        ...
        {
            string currBuildSettings = UnityEditor.PlayerSettings.GetScriptingDefineSymbolsForGroup(EditorUserBuildSettings.selectedBuildTargetGroup);

            //Check for and inject GAIA_PRESENT
            if (!currBuildSettings.Contains("GAIA_PRESENT"))
            {
                UnityEditor.PlayerSettings.SetScriptingDefineSymbolsForGroup(EditorUserBuildSettings.selectedBuildTargetGroup, currBuildSettings + ";GAIA_PRESENT");
            }
        }
