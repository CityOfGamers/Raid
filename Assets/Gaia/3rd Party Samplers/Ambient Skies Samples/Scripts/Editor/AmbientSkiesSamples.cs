...
namespace Gaia.GX.ProceduralWorlds
{
    public class AmbientSkiesSamples : MonoBehaviour
    {
        ...
        public static void GX_Skies_Morning()
        {
            ...
            SetUnderwaterFogSettings(UnityEngine.RenderSettings.fogColor, UnityEngine.RenderSettings.fogEndDistance);
        ...
        public static void GX_Skies_DefaultProcedural()
        {
            ...
            {
#if UNITY_EDITOR
                UnityEngine.RenderSettings.skybox = AssetDatabase.LoadAssetAtPath<Material>(skyMatPath);
                #endif
            }
            ...
            else
            {
                ...
                UnityEngine.RenderSettings.ambientSkyColor = GetColorFromHTML("B7C1C9");
                UnityEngine.RenderSettings.ambientEquatorColor = GetColorFromHTML("A9A395");
                UnityEngine.RenderSettings.ambientGroundColor = GetColorFromHTML("8E8166");
            }

            //Set the fog 
            UnityEngine.RenderSettings.fog = true;
            UnityEngine.RenderSettings.fogColor = new Color32(218, 224, 225, 255);
            UnityEngine.RenderSettings.fogMode = FogMode.Linear;
            UnityEngine.RenderSettings.fogStartDistance = -50f;
            Terrain theTerrain = GetActiveTerrain();
            if (theTerrain != null)
            {
                UnityEngine.RenderSettings.fogEndDistance = theTerrain.terrainData.size.y / 1.25f;
            }
            else
            {
                UnityEngine.RenderSettings.fogEndDistance = 1000f / 1.25f;
            }
        ...
        public static void BakeAmbientLight()
        {
            ...
            if (Lightmapping.lightingDataAsset != null)
            {
                Color Ambient = UnityEngine.RenderSettings.ambientSkyColor;
        ...
        private static void SetHDRISky(Vector3 sunRotation, string sunColor, float sunIntensity, string skyMaterial, string hdrSkyTexture, string skyTint, float skyExposure, float skyRotation, string fogColor, float fogStartDistance, float skyGroundIntensity, string skyColor, string equatorColor, string groundColor)
        {
            ...
            if (!string.IsNullOrEmpty(skyMatPath) && !string.IsNullOrEmpty(hdrSkyPath))
            {
                ...
                UnityEngine.RenderSettings.skybox = hdrSkyMaterial;
            }
#endif

            //Set render settings
            UnityEngine.RenderSettings.ambientMode = AmbientMode.Trilight;
            UnityEngine.RenderSettings.ambientSkyColor = GetColorFromHTML(skyColor);
            UnityEngine.RenderSettings.ambientEquatorColor = GetColorFromHTML(equatorColor);
            UnityEngine.RenderSettings.ambientGroundColor = GetColorFromHTML(groundColor);
            UnityEngine.RenderSettings.fog = true;
            UnityEngine.RenderSettings.fogColor = GetColorFromHTML(fogColor);
            UnityEngine.RenderSettings.fogMode = FogMode.Linear;
            UnityEngine.RenderSettings.fogStartDistance = -fogStartDistance;
            ...
            if (fogDistance < 250)
            {
                UnityEngine.RenderSettings.fogEndDistance = 1500f;
            }
            //If fogDistance great or equals to min clamp value it'll assign the value fogDistance and clamp it
            else
            {
                UnityEngine.RenderSettings.fogEndDistance = Mathf.Clamp(fogDistance, 250f, 2300f);
            }
